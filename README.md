...
Given the requirement for high throughput (concurrency) and accessibility of item in last n seconds or last n posted item (element in order), I considered using ConcurrentSkipListMap. The map is sorted in natural order of key or comparator passed which addresses usecase for ordered access in addition to concurrency.  The average complexity of this map is O(log n), support concurrency and implement ConcurrentNavigableMap which adds extensive ability to work with map for given usecases. Initially I thought of using ConcurrentHashMap(threadsafe (k,v)) and Queue/LinkList (order) to handle given usecases however ConcurrentSkipList data-structure provided similar to more functionality at avg O(log n) complexity.
